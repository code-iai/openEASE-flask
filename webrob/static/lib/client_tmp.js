...
class KnowrobClient {
    constructor(options) {
        ...
        function CanvasProxy() {
            this.viewer = function () {
                const ui = that.getActiveFrame().ui;
                if (!ui) { return undefined; }
                if (!ui.rosViewer) { return undefined; }
                return ui.rosViewer.rosViewer;
            };
            this.addMarker = function (marker, node) {
                if (this.viewer()) { this.viewer().addMarker(marker, node); }
            };
            this.removeMarker = function (marker, node) {
                if (this.viewer()) { this.viewer().removeMarker(marker, node); }
            };
        }
    ...
    registerNodes() {
        ...
        // Setup publisher that sends a dummy message in order to keep alive the socket connection
        {
            ...
            const keepAliveTopic = new ROSLIB.Topic({ ros: that.ros, name: '/keep_alive', messageType: 'std_msgs/Bool' });
            // A dummy message for the topic
            const keepAliveMsg = new ROSLIB.Message({ data: true });
            // Function that publishes the keep alive message
            const ping = function () { keepAliveTopic.publish(keepAliveMsg); };
        ...
        designatorClient.subscribe((message) => {
            if (message.description.length == 0) {
                console.warn('Ignoring empty designator.');
            } else {
                const desig_js = parse_designator(message.description);
                const html = format_designator(message.description);
                if (that.getActiveFrame().on_designator_received) { that.getActiveFrame().on_designator_received(html); }
            }
        });
        ...
        imageClient.subscribe((message) => {
            const ext = message.data.substr(message.data.lastIndexOf('.') + 1).toLowerCase();
        ...
        cameraPoseClient.subscribe((message) => {
            if (that.getActiveFrame().on_camera_pose_received) { that.getActiveFrame().on_camera_pose_received(message); }
        });

        // NOTE: frame windows may not be loaded already
        for (const i in user_interfaces) {
            const frame = document.getElementById(`${user_interfaces[i].id}-frame`);
            if (frame && frame.contentWindow && frame.contentWindow.on_register_nodes) { frame.contentWindow.on_register_nodes(); }
        }
        that.nodesRegistered = true;
    }

    waitForJsonProlog() {
        const client = new JsonProlog(that.ros, {});
        client.jsonQuery('true', (result) => {
            client.finishClient();
            if (result.error) {
                ...
                if (requireEpisode && !that.episode.hasEpisode()) { that.showPageOverlay('Please select an Episode'); }
    ...
    newCanvas(options) {
        const x = new KnowrobCanvas(that, options);
    ...
    selectMarker(marker) {
        if (that.selectedMarker == marker) { return; }
        ...
        if (that.getActiveFrame()) { that.getActiveFrame().selectMarker(marker); }
        // tell the webgl canvas to highlight the selected object
        if (that.canvas.viewer()) { that.canvas.viewer().highlight(marker); }
    }

    unselectMarker() {
        if (!that.selectedMarker) { return; }
        if (that.getActiveFrame() && that.getActiveFrame().unselectMarker) { that.getActiveFrame().unselectMarker(that.selectedMarker); }
        // tell the webgl canvas to unhighlight the object
        if (that.canvas.viewer()) { that.canvas.viewer().unhighlight(that.selectedMarker); }
    ...
    removeMarker(marker) {
        ...
        if (that.getActiveFrame() && that.getActiveFrame().removeMarker) { that.getActiveFrame().removeMarker(marker); }
    }

    showMarkerMenu(marker) {
        if (that.getActiveFrame() && that.getActiveFrame().showMarkerMenu) { that.getActiveFrame().showMarkerMenu(marker); }
    }

    on_render(camera, scene) {
        if (that.getActiveFrame() && that.getActiveFrame().on_render) { that.getActiveFrame().on_render(camera, scene); }
    ...
    on_episode_selected(library) {
        for (const i in user_interfaces) {
            const frame = document.getElementById(`${user_interfaces[i].id}-frame`);
            if (frame && frame.contentWindow.on_episode_selected) { frame.contentWindow.on_episode_selected(library); }
        }
    ...
    showFrame(iface_name) {
        const frame_name = getInterfaceFrameName(iface_name);
        ...
        const new_src = getInterfaceSrc(iface_name);
        const frame = document.getElementById(`${frame_name}-frame`);
        const old_src = frame.src;
        if (!old_src.endsWith(new_src)) {
            frame.src = new_src;
            if (frame.contentWindow && frame.contentWindow.on_register_nodes) { frame.contentWindow.on_register_nodes(); }
        }
    ...
    getActiveFrame() {
        const frame = document.getElementById(`${getActiveFrameName()}-frame`);
    ...
    getInterfaceFrameName(iface) {
        for (const i in user_interfaces) {
            const elem = user_interfaces[i];
            if (elem.id == iface) return elem.id;
            for (const j in elem.interfaces) {
    ...
    getActiveInterfaceName() {
        for (const i in user_interfaces_flat) {
    ...
    updateQueryString() {
        urlQuery = {};
        const query = String(window.location.hash.substring(1));
        ...
        for (let i = 0; i < vars.length; i++) {
            ...
            if (typeof urlQuery[pair[0]] === 'undefined') {
                ...
                const arr = [urlQuery[pair[0]], decodeURIComponent(pair[1])];
                urlQuery[pair[0]] = arr;
            } else {
