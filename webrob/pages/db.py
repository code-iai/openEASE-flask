...
def db_docu_text(key):
    ...
    __display_warning_when_unable_to_find_docu(key)
...
def __display_warning_when_unable_to_find_docu(key):
...
def db_save_user_roles():
    ...
    if 'roles' in data and 'id' in data:
        __save_roles_to_users_and_commit_to_db(data)
    else:
        app.logger.warn("Invalid input data.")
    return jsonify(result=None)


def __save_roles_to_users_and_commit_to_db(data):
    db_adapter = current_app.user_manager.db_adapter
